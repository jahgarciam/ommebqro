...
export const metadata = {
  title: "Recursos públicos | OMMEB_Qro",
  description:
    "Recursos públicos de introducción para el entrenamiento olímpico en álgebra, combinatoria, geometría y teoría de números.",
};
...
function PublicAdRail({ side }: { side: "left" | "right" }) {
  const placement = side === "left" ? "resources-left" : "resources-right";

  return (
    <aside className="hidden 2xl:block">
      <div className="sticky top-8 min-h-[720px] rounded-3xl bg-white p-4 shadow-sm">
        <AdSlot placement={placement} className="min-h-[640px] w-full" />
      </div>
    </aside>
  );
}
    ...
    <section className="w-full">
      <header className="rounded-3xl bg-white p-6 shadow-sm md:p-8">
        ...
        <p className="mt-4 max-w-3xl text-lg leading-8 text-slate-700">
          Estos recursos no sustituyen la plataforma completa, pero permiten
          conocer el tipo de razonamiento que se busca desarrollar: observar
          patrones, justificar procedimientos, organizar casos y construir
          argumentos.
        </p>
      </header>
...
export default function RecursosPage() {
  ...
  return (
    <main className="min-h-screen bg-[#F5F5F5]">
      <div className="mx-auto grid max-w-[1600px] grid-cols-1 gap-6 px-6 py-8 2xl:grid-cols-[280px_minmax(0,900px)_280px]">
        <PublicAdRail side="left" />
        <RecursosContent />
        <PublicAdRail side="right" />
      </div>
    </main>
  );
}